...
def _get_trials(analytics_count: int) -> int:
    mapping_dict = {
        analytics_count < 5_000: 70,
        analytics_count < 10_000: 65,
        analytics_count < 100_000: 55,
        analytics_count <= 500_000: 45,
        analytics_count > 500_000: 25
    }
...
async def optimize_regression_model(
    ...
    study.optimize(objective, n_trials=_get_trials(analytics_count), n_jobs=-1, timeout=60 * 30)  # 30 minutes

    # train final model
    lgbm_model = lgbm.LGBMRegressor(**study.best_params)
    lgbm_model.fit(X_train, y_train, eval_set=[(X_val, y_val)],
                   callbacks=[lgbm.callback.early_stopping(25, verbose=False)])
